...
def SK1(pojo_proactive: CombatPojo, pojo_reactive: CombatPojo) -> (str, bool):
    if not make_decision(0.2):
        return "", False
    content = ""
    base_attack = max(pojo_proactive.attack - pojo_reactive.defense, 1) * random.uniform(1.1, 1.2) * 1.3
...
normal_items = [ItemNormal("N1", "经验药水", "获得200点经验", 500,  N1),
                ItemNormal("N2", "经验丹", "永久提升1%的经验加成", 1000000, N2),
...
equip_items = [ItemEquip("EQ1", "树棍", "地上捡的树枝做的棍子", 1000, 0, StatusAdd(attack=50)),
               ...
               ItemEquip("EQ8", "圣剑", "传说中的勇者用过的剑", 10000000, 0,
                         StatusAdd(attack=2000, critical_strike=0.5, critical_damage=2)),
               ItemEquip("EQ9", "圣盔", "传说中的勇者戴过的头盔", 10000000, 1, StatusAdd(defense=500)),
               ItemEquip("EQ10", "圣者上装", "传说中的勇者穿过的衣服", 10000000,  2,
                         StatusAdd(defense=1000, blood_max=5000)),
               ItemEquip("EQ11", "圣者下装", "传说中的勇者穿过的裤子", 10000000,  3, StatusAdd(defense=500)),
               ItemEquip("EQ12", "圣速鞋", "传说中的勇者穿过的鞋子", 10000000,  4, StatusAdd(speed=20)),
               ...
               ItemEquip("EQ15", "雷霆枪", "蕴含雷霆神力的枪", 500000, 0,
                         StatusAdd(attack=250, critical_strike=0.1, critical_damage=0.2)),
...
skill_items = [ItemSkill("SK1", "全力一击", "攻击时触发,触发概率:20%\n消耗魔力:100\n伤害加成:30%", 20000, on_attack=SK1)]
all_items = [*special_items, *normal_items, *skill_items, *equip_items]

# 上架商店的物品
on_store_items = {"SP1", "N1", "N2", "N3", "N4", "N5", "N6", "N7", "N8", "N9", "EQ1", "EQ2", "EQ3", "EQ4", "EQ5", "EQ6", "EQ7", "SK1", "EQ8"}
store_items = []
...
tower_monster_dropping = ["N1", "N2", "N3", "N4", "N5", "N6", "N7", "N8", "N9", "SP1"]
tower_monster_dropping_p = [0.02, 0.0002, 0.1, 0.1, 0.04, 0.04, 0.02, 0.02, 0.005, 0.1]
for i in range(len(tower_monster_dropping)):
    tower_monster_dropping[i] = items_mp_by_id[tower_monster_dropping[i]]
